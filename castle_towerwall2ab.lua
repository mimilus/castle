...
for _, row in ipairs(towerwall2ab.types) do
	local name = row[1]
	local desc = row[2]
	local tile = row[3]
	...
	node_box = {
		...
		fixed = {
			...
			{-0.375,-0.5,-0.4375,-0.25,0.5,0.5}, --NodeBox2
			{-0.25,-0.5,-0.375,-0.125,0.5,0.5}, --NodeBox3
			{-0.125,-0.5,-0.3125,0,0.5,0.5}, --NodeBox4
			{0,-0.5,-0.25,0.125,0.5,0.5}, --NodeBox5
			{0.125,-0.5,-0.1875,0.25,0.5,0.5}, --NodeBox6
			{0.25,-0.5,-0.125,0.375,0.5,0.5}, --NodeBox7
...
--MIRROR
	-- TowerWall low angle Part1
	minetest.register_node("castle:" ..name.. "towerwallpartonemirror", {
	    drawtype = "nodebox",
		description = desc.." Tower Wall Part One Mirror",
		tiles = {tile},
		groups = {cracky=3,attached_node=0},
		sounds = default.node_sound_stone_defaults(),
	    paramtype = "light",
	...
	if craft_material then
		--Choose craft material
		minetest.register_craft({
			output = "castle:" ..name.. "towerwallpartonemirror 4",
			recipe = {
			{"",craft_material,""},
			{"",craft_material,""},
			{craft_material,craft_material,craft_material} },
	...
	-- TowerWall low angle Part2
	minetest.register_node("castle:" ..name.. "towerwallparttwomirror", {
	    drawtype = "nodebox",
		description = desc.." Tower Wall Part Two Mirror",
		tiles = {tile},
		groups = {cracky=3,attached_node=0},
		sounds = default.node_sound_stone_defaults(),
	    paramtype = "light",
	...
	if craft_material then
		--Choose craft material
		minetest.register_craft({
			output = "castle:" ..name.. "towerwallparttwomirror 4",
			recipe = {
			{"",craft_material,""},
			{"",craft_material,""},
			{craft_material,craft_material,craft_material} },
