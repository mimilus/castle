...
for _, row in ipairs(sidepillar.types) do
	local name = row[1]
	local desc = row[2]
	local tile = row[3]
	local craft_material = row[4]
	-- Node Definition
	minetest.register_node("castle:side_pillars_"..name.."_bottom", {
	    drawtype = "nodebox",
		description = desc.." Side Pillar Base",
		tiles = {tile},
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-0.5,-0.5,-0.40625,0.3125,-0.375,0.40625},
			{-0.5,-0.375,-0.25,0,-0.125,0.25},
			{-0.5,-0.125,-0.15625,-0.1875,0.5,0.15625},
		},
	},
	})
	minetest.register_node("castle:side_pillars_"..name.."_top", {
	    drawtype = "nodebox",
		description = desc.." Side Pillar Top",
		tiles = {tile},
		groups = {cracky=3,attached_node=1},
		sounds = default.node_sound_stone_defaults(),
	    paramtype = "light",
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-0.5,0.375,-0.40625,0.3125,0.5,0.40625},
			{-0.5,0.125,-0.25,0,0.375,0.25},
			{-0.5,-0.5,-0.15625,-0.1875,0.125,0.15625},
		},
	},
	})
	minetest.register_node("castle:side_pillars_"..name.."_middle", {
	    drawtype = "nodebox",
		description = desc.." Side Pillar Middle",
		tiles = {tile},
		groups = {cracky=3,attached_node=1},
		sounds = default.node_sound_stone_defaults(),
	    paramtype = "light",
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-0.5,-0.5,-0.15625,-0.1875,0.5,0.15625},
		},
	},
	})
	if craft_material then
		...
		minetest.register_craft({
			output = "castle:side_pillars_"..name.."_bottom 4",
			recipe = {
			{"",craft_material,""},
			{"",craft_material,""},
	...
	if craft_material then
		--Choose craft material
		minetest.register_craft({
			output = "castle:side_pillars_"..name.."_top 4",
			recipe = {
			{craft_material,craft_material,craft_material},
			{"",craft_material,""},
			{"",craft_material,""} },
	...
	if craft_material then
		--Choose craft material
		minetest.register_craft({
			output = "castle:side_pillars_"..name.."_middle 4",
			recipe = {
